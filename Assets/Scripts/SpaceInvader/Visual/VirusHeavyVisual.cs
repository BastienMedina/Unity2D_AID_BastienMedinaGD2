...
// Script autonome de visuel pour le virus lourd
public class VirusHeavyVisual : MonoBehaviour
{
    // Couleur rouge foncé du virus lourd
    [SerializeField] private Color _color = new Color(0.8f, 0.2f, 0.2f, 1f);

    // Échelle horizontale du virus en unités monde
    [SerializeField] private float _scaleX = 0.5f;

    // Échelle verticale du virus en unités monde
    [SerializeField] private float _scaleY = 0.4f;

    // Ordre de tri dans le layer de rendu par défaut
    [SerializeField] private int _sortingOrder = 2;

    // Rayon local du collider pour obtenir 0.35u en espace monde
    [SerializeField] private float _colliderRadius = 0.7f;
    ...
    // Initialise le visuel et le collider du virus lourd
    private void Awake()
    {
